...
public class MealInfo {
    ...
    public void initialize() {
    	descriptiontext.setText("Product: "+(cache.getCurrent_meal().getID()-1) + "\n" + cache.getCurrent_meal().getName() + "\n" + "Description: " + 
    cache.getCurrent_meal().getDesc() + "\n" + "Price: " + cache.getCurrent_meal().getPrice());
    	
    	List<Ingredients> base = cache.getCurrent_meal().getBaseIng();
    ...
    void update_return(ActionEvent event) throws IOException {
    	...
    	cache.getCurrent_meal().setNewIng(listViewData_optinal);
